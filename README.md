...
| `SESSION_ROTATE_MAX_MB` | `20` | Managed-session transcript size (MB) at which the device rotates to a fresh session, so a long-lived persistent session never grows into the resume-from-summary modal that wedges a headless bridge. Ignored when `RESUME_SESSION_ID` is pinned |
| `SESSION_RETENTION_KEEP` | `4` | How many newest transcripts to keep on disk (current session + 3 prior); older ones are pruned so the persistent volume / backup mirror does not grow without bound |
| `STARTUP_WEDGE_ENABLED` | `true` | Per-launch backstop for a relay that never connects (e.g. a resume modal blocking startup, which the rx-stall watchdog cannot see). If `/healthz` is not 200 within the grace window while claude is alive, relaunch the same session once; on a second consecutive failure rotate to a fresh session and fire a Telegram alert. Set `false` to disable |
| `STARTUP_WEDGE_GRACE_SECONDS` | `75` | How long after a launch to wait for the relay to come up before the startup-wedge backstop acts |
