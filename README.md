...
| `RESUME_SESSION_ID` | _(unset)_ | Pin an explicit session UUID to resume, **bypassing managed rotation** (the size cap below is not applied). When unset, the device manages its own session id in a state file, resuming it across restarts and rotating to a fresh one once the transcript exceeds `SESSION_ROTATE_MAX_MB`; on first run with no state file it adopts the newest transcript on disk so an existing session is preserved |
| `SESSION_ROTATE_MAX_MB` | `20` | Managed-session transcript size (MB, **positive integer**) at which the device rotates to a fresh session, so a long-lived persistent session never grows into the resume-from-summary modal that wedges a headless bridge. Ignored when `RESUME_SESSION_ID` is pinned. An invalid value falls back to the default with a warning |
| `SESSION_RETENTION_KEEP` | `4` | How many newest transcripts to keep on disk (**positive integer**; current session + 3 prior); older ones are pruned so the persistent volume / backup mirror does not grow without bound. An invalid value falls back to the default with a warning |
| `STARTUP_WEDGE_ENABLED` | `true` | Per-launch backstop for a relay that never connects (e.g. a resume modal blocking startup, which the rx-stall watchdog cannot see). If `/healthz` is not 200 within the grace window while claude is alive, relaunch the same session once; on a second consecutive failure rotate to a fresh session and fire a Telegram alert (a pinned `RESUME_SESSION_ID` session is never rotated away). Set `false` to disable |
| `STARTUP_WEDGE_GRACE_SECONDS` | `75` | How long (**positive integer**, seconds) after a launch to wait for the relay to come up before the startup-wedge backstop acts. An invalid value falls back to the default with a warning |
| `TELEGRAM_BOT_TOKEN` / `STARTUP_WEDGE_ALERT_CHAT_ID` | _(unset)_ | Optional. When both are set, the startup-wedge backstop sends a Telegram alert on a forced fresh-session recovery. Best-effort; unset = log only |
